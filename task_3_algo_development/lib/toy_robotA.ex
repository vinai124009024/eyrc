defmodule CLI.ToyRobotA do
  ...
  def stop(robot, goal_locs, cli_proc_name) do
    ###########################
    ## complete this funcion ## stop 4,c | 3,e | 3,b | 1,c
    ###########################
    ...
    Process.register(pid_client, :client_toyrobotA)
    receive do
      {:ok, robot} ->
        {:ok, robot}
    end
  ...
  def efficient_reach(robot, goal_locs, cli_proc_name) do
    ...
    Process.sleep(500)
    send(:client_toyrobotB, {:toyrobotA_n, n})
    receive do
      {:toyrobotB_n, n2} ->
        cond do
          n <= n2 -> 
            ...
            send(:client_toyrobotB, {:toyrobotA_er_over, true})
            robot
          true -> 
            send_robot_status(robot, cli_proc_name)
            receive do
  ...
  def reach_all_goals(robot, goal_locs, cli_proc_name) do
    ...
    if Enum.empty?(goal_locs) do
      send_robot_status(robot, cli_proc_name)
      send(:client_toyrobotB, {:toyrobotA_over, true})
      receive do
        {:toyrobotB_over, done} ->
          nil
      end
      ...
      send(:client_toyrobotB, {:toyrobotA_goal, [Integer.to_string(gx), Atom.to_string(gy)]})
      robot = reach_goal(robot, gx, gy, cli_proc_name, [:straight])
      reach_all_goals(robot, goal_locs, cli_proc_name)
    end
  end
  ...
  def check_for_robot(robot) do
    tr = nil
    send(:client_toyrobotB, {:toyrobotA_status, robot})
    tr = receive do
      {:toyrobotB_status, robotb} ->
        move(robotb)
      after
        0 -> nil
    end
