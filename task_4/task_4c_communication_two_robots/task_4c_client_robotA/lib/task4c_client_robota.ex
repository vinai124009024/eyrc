defmodule Task4CClientRobotA do
  ...
  def main do
    ...
    ###########################
    ## complete this funcion ## Task4CClientRobotA.main
    ###########################
    [ch, ch2] = Task4CClientRobotA.PhoenixSocketClient.connect_server()
    goal_locs = Task4CClientRobotA.PhoenixSocketClient.get_goals(ch)
    gl = Enum.map(goal_locs, fn g -> g["pos"] end)
    Task4CClientRobotA.PhoenixSocketClient.init_server_variables(ch, gl)
    start_pos = Task4CClientRobotA.PhoenixSocketClient.get_start_pos(ch) 
  ...
  def efficient_reach(robot, goal_locs, cli_proc_name, ch2) do
    ...
    Process.sleep(500)
    #Task4CClientRobotA.PhoenixSocketClient.send_to_b(ch2, {:toyrobotA_n, n})
    receive do
      {:toyrobotB_n, n2} ->
        cond do
          n <= n2 -> 
            robot = reach_goal(robot, gx, gy, cli_proc_name, ch2, [:straight])
            {:obstacle_presence, obs} = Task4CClientRobotA.PhoenixSocketClient.send_robot_status(cli_proc_name, robot)
            #Task4CClientRobotA.PhoenixSocketClient.send_to_b(ch2, {:toyrobotA_er_over, true})
            robot
          true -> 
            {:obstacle_presence, obs} = Task4CClientRobotA.PhoenixSocketClient.send_robot_status(cli_proc_name, robot)
            receive do
  ...
  def reach_all_goals(robot, goal_locs, cli_proc_name, ch2) do
    goal_locs = Task4CClientRobotA.PhoenixSocketClient.get_updated_goals(cli_proc_name)
    if Enum.empty?(goal_locs) do
      {:obstacle_presence, obs} = Task4CClientRobotA.PhoenixSocketClient.send_robot_status(cli_proc_name, robot)
      Task4CClientRobotA.PhoenixSocketClient.stop_process(cli_proc_name)
      {:ok, robot}
    else
      [gx, gy] = find_closest(robot, goal_locs)
      Task4CClientRobotA.PhoenixSocketClient.update_goals(cli_proc_name, [gx, gy])
      robot = reach_goal(robot, gx, gy, cli_proc_name, ch2, [:straight])
  ...
  def check_for_robot(robot, ch) do
    pos = Task4CClientRobotA.PhoenixSocketClient.get_b_pos(ch) 
    tr = move(robot)
